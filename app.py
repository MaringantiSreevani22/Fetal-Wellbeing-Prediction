...
# Fetal Development page route
@app.route("/fetal-development")
def fetal_development():
    return render_template("fetal-development.html")

# Pregnancy Care page route
@app.route("/pregnancy-care")
def pregnancy_care():
    return render_template("pregnancy-care.html")

# Inspect or Join page
@app.route("/home", methods=["GET", "POST"])
def inspect():
    if request.method == "POST":
        # Handle form submission and redirect to the prediction route
...
def predict():
    if request.method == 'POST':
        ...
        out = ['Normal', 'Pathological', 'Suspect']
        result = out[int(output[0])]

        return render_template('output.html', result=result)

# View Report page route
